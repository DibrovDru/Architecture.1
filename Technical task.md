...
3. Реализовать возможность ведения задач по каждому проекту.
...
Из-за отсутствия регистрации и авторизации пользователей нет возможности отстлеживать автора внесения изменений и просмотра истории работы по задачам.
...
### Для кого разработано веб-приложение:

 - Тем,кто ведет учет данных по задачам в таблицах или на листе бумаги. 
Присутствует возможность создания,назначения и ведения задач по каждому сотруднику.

- Тем, кто бережно относится к своим данным и беспокоится об их безопасности. 
Веб-приложение гарантирует сохранность данных .

- Тем, кто работает с большим объемом задач.
В приложении реализуется возможность распределения задач по статусу, а так же поиск и использование фильтров для быстрого поиска.

- Тем, кому необходимо отслеживание работы по каждому сотруднику.
Веб приложении реализуется регистрация и авторизация пользователей, с возможностью отслеживания автора изменений и просмотра комментариев по проекту.
...
Если у пользователя проект уже есть открываем страницу с *выбором проекта*.
...
Эта архитектура является традиционным решением для создания приложений, но некоторые разработчики считают ее устаревшей. Тем не менее, мы считаем, что монолитная архитектура является идеальным решением.
...
SPA разрабатываются на стороне клиента (рендеринг на стороне клиента) с использованием фреймворков JavaScript, так как вся логика всегда переносится на фронтенд.
...
Пользовательское тестирование приложения, исправление выявленных ошибок, оптимизация.
